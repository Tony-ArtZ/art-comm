      ...
      <nav className="flex flex-row items-center justify-between w-screen px-4 py-5 sm:bg-none md:shadow-none bg-secondary sm:px-8">
        ...
        <div className=" h-11 sm:h-12 2xl:h-14 absolute left-0 right-0 sm:ml-auto sm:mr-auto ml-20 mr-auto xl:w-[400px] 2xl:w-[456px] sm:w-[350px] w-[60vw] flex flex-row justify-center items-center">
          <input
            ...
            className=" bg-primary text-white md:text-xl text-sm font-inter shadow-glowLow rounded-[40px] h-full pl-6 w-full outline-none "
          />
          ...
          <button
            onClick={() => search()}
            className="absolute right-0 flex items-center justify-center h-full text-xl text-white rounded-full w-11 shadow-glowLow 2xl:w-14 sm:w-12"
          >
            <FaSearch
              fontSize="24px"
        ...
        <ul className="absolute flex flex-row mt-0 md:mt-2 gap-2 2xl:gap-8 xl:gap-4 sm:mt-3 xl:right-24 sm:right-8 lg:right-12 right-4">
          {user ? (
            <>
              ...
              <button className="hidden md:list-item">Signout</button>
              <Link href={`/profile/${user?.id}`}>
                <Image
                  ...
                  height={24}
                  alt="Profile"
                  className="p-0 m-0 mr-1 border-2 rounded-full w-14 h-14 border-interactive drop-shadow-glow md:hidden"
