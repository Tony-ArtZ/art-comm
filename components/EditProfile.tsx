...
export default function EditProfile ({user, userData, profileEditingHandler}: {user: User, userData: any, profileEditingHandler: (value:boolean)=>void}) {
  ...
  const handleBannerChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log(fileSelected, fileReader)
  ...
  return (
    <div className="fixed z-[100] w-screen h-screen">
      ...
      <main className="absolute top-0 bottom-0 left-0 right-0 z-50 flex flex-col items-center p-2 m-auto border-4 border-solid rounded-xl h-fit border-interactive bg-primary">
        ...
        <input id="bannerInput" onChange={handleBannerChange} type="file" accept="image/png, image/jpeg" className="absolute top-[1000] -z-20 hidden"/>
          <label htmlFor="bannerInput" className="w-full max-h-full min-h-full relaticm">
        <div className="absolute z-20 p-2 text-2xl text-center border-2 border-solid rounded-full top-2 right-2 grid place-items-center bg-secondary drop-shadow-glowHigh text-interactive border-interactive">
         ...
         {userData.banner_picture && (
          ...
          <input id="profileInput" onChange={handleProfileChange} type="file" accept="image/png, image/jpeg" className="absolute top-[1000] -z-20 hidden"/>
          <label 
